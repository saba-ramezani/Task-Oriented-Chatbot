...
def get_type_of_two_barracks_link(barracks_id_1, barracks_id_2):
    ...
    rows = cursor.fetchall()
    if rows:
        output = "ارتباط پادگان " + str(barracks_id_1) + " با پادگان " + str(barracks_id_2) + " از نوع " + str(rows[0][0]) + " میباشد."
        print(output)
        cursor.close()
        return output
    else:
        output = "داده ای در رابطه با نوع ارتباط پادگان " + str(barracks_id_1) + " با پادگان " + str(barracks_id_2) + " یافت نشد."
        print(output)
        cursor.close()
        return output
